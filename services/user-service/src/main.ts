...
async function bootstrap() {
  ...
  express.get("/metrics", (_req, res) => {
    ...
    const lines = [
      ...
      `app_requests_total{service="user-service"} ${totalRequests}`,
    ...
    for (const [route, metric] of pathMetrics.entries()) {
      ...
      lines.push(`app_request_duration_ms_sum{service="user-service",route="${safeRoute}"} ${metric.sumMs}`);
      lines.push(`app_request_duration_ms_count{service="user-service",route="${safeRoute}"} ${metric.count}`);
      lines.push(`app_request_duration_ms_max{service="user-service",route="${safeRoute}"} ${metric.maxMs}`);
    }

    for (const [statusCode, count] of statusCounters.entries()) {
      lines.push(`app_requests_status_total{service="user-service",status_code="${statusCode}"} ${count}`);
    }
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription("OpenAPI contract for user-service")
    .setVersion("1.0.0")
    .addBearerAuth()
    .build();
