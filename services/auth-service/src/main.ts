...
async function bootstrap() {
  ...
  express.get("/metrics", (_req, res) => {
    ...
    const lines = [
      ...
      `app_requests_total{service="auth-service"} ${totalRequests}`,
    ...
    for (const [route, metric] of pathMetrics.entries()) {
      ...
      lines.push(`app_request_duration_ms_sum{service="auth-service",route="${safeRoute}"} ${metric.sumMs}`);
      lines.push(`app_request_duration_ms_count{service="auth-service",route="${safeRoute}"} ${metric.count}`);
      lines.push(`app_request_duration_ms_max{service="auth-service",route="${safeRoute}"} ${metric.maxMs}`);
    }

    for (const [statusCode, count] of statusCounters.entries()) {
      lines.push(`app_requests_status_total{service="auth-service",status_code="${statusCode}"} ${count}`);
    }
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription("OpenAPI contract for auth-service")
    .setVersion("1.0.0")
    .addBearerAuth()
    .build();
